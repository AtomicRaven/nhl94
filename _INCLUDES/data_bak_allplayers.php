...
function DuplicateRosterTable($newTable){
	...
	//Copy Table Data
	$sql = "INSERT $newTable SELECT * FROM roster";	
	$tmr = mysqli_query($conn, $sql);

	if ($tmr) {
		logMsg("Table Roster Data Copied into: " . $newTable);
	} else {
		echo("<br>Error: DuplicateRosterTable: Copy " . $sql . "<br>" . mysqli_error($conn));
	}
}
...
function TransferPlayerRoster($newTable){
	...
	foreach($arrResult as $value) {		
		...
		if($value[0] != ""){
			...
			$firstname = $name[0];
			$lastname = $name[1];
			$teamid = GetTeamIdByABV($value[1]);				
			
			//$update = "UPDATE $newTable SET TeamID = '$teamid', Team='$value[1]', PlayerID='$i' WHERE (First = '$firstname' AND Last = '$lastname')";
			$update = "UPDATE $newTable SET TeamID = '$teamid', Team='$value[1]' WHERE (First = '$firstname' AND Last = '$lastname')";
			//echo "sql:" . $update . "<br/>";
			
			$sqlr = mysqli_query($conn, $update);
			
			if ($sqlr) {
				echo($i . " Player: " . $value[0] . " moved to team name: " . $value[1] . " teamId: " .$teamid . "<br/>");
			} else {
...
function GetPlayerStatsBySeriesId($seriesid, $pos, $leagueid){	
	...
	$sql = "SELECT s.*, p.*, 
		...
		ON s.GameID = p.GameID WHERE (s.SeriesID = '$seriesid' AND p.LeagueID='$leagueid') 
		GROUP BY p.PlayerID ";
